...
| `force_background_bash.py` | Bash | Sync-timeout policy for the main agent / teammates: a requested timeout > 60s is silently clamped to 60s when the CLI can auto-background the command at timeout (so the conversation stops blocking, nothing is killed), and *denied with advice* when it can't — `$VAR`/backtick redirect targets, `git`, leading `sleep`, unquoted heredocs, `<<'EOF' > out` — because for those the sync timeout is a hard SIGTERM. Subagents may not use `run_in_background` (ending their turn returns them to the parent, so they can't wait for the completion notification) unless the command carries `BACKGROUND_NEEDED`; in-process teammates are told apart from subagents by `utils/_agent_kind.py`. Kill-class fixtures probed live against the CLI in `tests/test_force_background_bash.py`; agent classification and the clamp in `tests/test_agent_kind.py`. |
