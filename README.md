...
|---|---|---|
| `security_guard.py` | Bash | Two stages. Box-destruction patterns (rm -rf on root/home, mkfs, dd onto a disk, …) are denied (`permissionDecision: "deny"`), the agent loop is stopped (`continue: false`) and the human's ntfy hotline (`CLAUDE_HOTLINE_NTFY_TOPIC`, skipped when unset) gets host / cwd / session id / full command — `continue: false` alone does *not* block, the tool runs first. The "download piped into a shell" pattern instead goes to a judge: the script is fetched to `$TMPDIR/claude-guard/` and a restricted nested `claude -p` (no settings, no MCP, WebSearch as its only tool, subscription auth, schema-enforced verdict) running `agents/remote-script-judge.md` decides. Allowed → the command runs with the verdict as `additionalContext`; concern or judge unavailable → `ask` plus a hotline ping (`deny` in `bypassPermissions`/`dontAsk`, where nobody answers prompts); the judge's optional note is forwarded over ntfy (`CLAUDE_NTFY_TOPIC`) either way. Needs `claude` on PATH and logged in; `CLAUDE_GUARD_JUDGE_CMD` / `CLAUDE_GUARD_SKIP_FETCH` stub it for tests. |
