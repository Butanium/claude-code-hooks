...
| `session_env.py` | Writes lines to `$CLAUDE_ENV_FILE`, which the CLI sources before every Bash tool command: `unset -f grep find` (the CLI's ugrep/bfs shell functions skip gitignored dirs and fail on common GNU usage, silently inside compound commands; `CLAUDE_HOOKS_KEEP_CC_SEARCH=1` keeps them), and `. "$CLAUDE_SECRETS_FILE"` when that variable names a file, so a session launched from a long-lived tmux server gets the file's current keys instead of the ones the server started with. Never reads or prints values. `tests/test_session_env.py`; live: `tests/smoke_session_env.sh`. |
